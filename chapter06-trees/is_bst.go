...
func IsBST3(root *BSTNode, min, max int) bool {
	if root == nil {
		return true
	}
	return (root.data > min && root.data < max && IsBST3(root.left, min, root.data) && IsBST3(root.right, root.data, max))
}

func IsBST4(root *BSTNode, prev *int) bool {
	if root == nil {
		return true
	}

	if !IsBST4(root.left, prev) {
		return false
	}
	if root.data < *prev {
		return false
...
func main() {
	...
	fmt.Println(IsBST1(t1))
	fmt.Println(IsBST2(t1))
	fmt.Println(IsBST3(t1, math.MinInt32, math.MaxInt32))
	var prev = math.MinInt32
	fmt.Println(IsBST4(t1, &prev))
