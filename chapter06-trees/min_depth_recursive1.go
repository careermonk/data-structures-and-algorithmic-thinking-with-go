...
func MinDepth(root *BinaryTreeNode) int {
	...
	if root.left == nil { // If left subtree is NULL, find minimum depth in right subtree
		return MinDepth(root.right) + 1
	}

	if root.right == nil { // If right subtree is NULL, find minimum depth in left subtree
